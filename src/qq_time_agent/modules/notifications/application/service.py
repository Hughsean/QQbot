...
class NotificationService:
    ...
    async def send_confirmation(
        ...
        if slot is None:
            ...
            body = (
                f"建议《{proposal.title}》: {_time(slot.starts_at, self._owner_timezone)} 至 "
                f"{_time(slot.ends_at, self._owner_timezone)}。"
    ...
    async def send_reminder(
        ...
        body = (
            f"提醒:《{entry.title}》将在 {_time(entry.starts_at, self._owner_timezone)} 开始。"
