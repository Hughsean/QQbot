...
数据库要经 Compose 服务名 `postgres` 访问，Ollama 经 Compose 服务名 `ollama` 访问。任何一处直接放开都会破坏 ADR-0010
确立的"无公网入口"不变量。
...
- 引入显式 `APP_CONTAINER` 开关（默认 `false`）。容器模式只接受三组精确值：
  ...
  `OLLAMA_BASE_URL=http://ollama:11434`；任何其他值启动失败。裸机模式保持
  原有回环强门禁不变。
...
- Ollama 作为 Compose 服务运行，只加入私有网络且不发布宿主机端口。模型保存在独立命名卷，
  `ollama-init` 在应用启动前确保 `qwen3-embedding:4b` 已存在。GPU 模式使用 NVIDIA
  Container Toolkit；无可用 GPU 时可通过 CPU override 启动，但 readiness 仍必须通过。
...
- 容器内 `0.0.0.0` 绑定只作用于 Compose 私有网络，主机侧可达性仍由 `127.0.0.1` 发布和
  Host 头校验双重限制。
- 依赖 Ubuntu Docker Engine、Compose 和（GPU 模式）NVIDIA Container Toolkit 持续运行；
  Ollama 不得发布到局域网。
...
- 容器内可访问 `ollama:11434`，模型初始化任务成功，Ollama health 为 available。
