...
- Git 提交消息必须使用中文，遵循 Conventional Commits 类型前缀格式。
- 格式：`<type>(<scope>): <中文描述>`，例如：`feat(version): 添加部署版本信息`。
- `type` 使用 `feat`、`fix`、`docs`、`refactor`、`test`、`chore` 等标准类型；`scope` 应简洁、准确地标明变更范围。
- 描述应直接说明实际变更，使用中文，不要使用无意义的“更新代码”“修复问题”等笼统表述。
